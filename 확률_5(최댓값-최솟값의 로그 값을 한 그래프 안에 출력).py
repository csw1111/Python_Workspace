...
ax1.set_xticks(range(1, 101, 1000))
...
ax2.plot(x1, y4, label='최대 수익 - 최소수익(로그)', color='brown', linestyle=':')
ax2.set_ylabel('최대/최소 수익', color='black')
ax2.tick_params(axis='y', labelcolor='black')

y2_ticks = np.linspace(minlst[0],maxlst[0])
...
for i in range(top_10_percent):
    ...
    ax2.annotate(f'{y4_sorted[-(i+1)]:.2f}', xy=(x1[np.where(y4 == y4_sorted[-(i+1)])][0], y4_sorted[-(i+1)]),
