...
for x in range(100):
    ...
    for y in range(int(rept/2)):
        if rlst[x] == mnlst1[x][-(y+1)] - mnlst1[x][y]:
            c25lst[x] += 1
    print(minlst[x], maxlst[x], rlst[x], c25lst[x])
...
ax1.plot(x1, y1, label='가장 큰 수익을 가진 퍼센트 개수', color='blue', linestyle='--')
ax1.set_xlabel('x')
...
ax2.plot(x1, y2, label='수익', color='red', linestyle='-')
ax2.set_ylabel('수익', color='black')
ax2.tick_params(axis='y', labelcolor='black')              

ax3 = ax1.twinx()
ax3.plot(x1, y3, label='최댓값-최솟값', color='black', linestyle='-')
ax3.spines['right'].set_position(('outward', 60))  # 세 번째 그래프를 오른쪽에 배치
ax3.set_ylabel('최댓값-최솟값', color='black')
ax3.tick_params(axis='y', labelcolor='black')

ax4 = ax1.twinx()
ax4.plot(x1, y4, label='매매한 횟수', color='brown', linestyle='-')
ax4.spines['right'].set_position(('outward', 120))  # 세 번째 그래프를 오른쪽에 배치
ax4.set_ylabel('매매한 횟수', color='black')
ax4.tick_params(axis='y', labelcolor='black')

ax5 = ax1.twinx()
ax5.plot(x1, y5, label='최저값과 최댓값이 나온 개수', color='c', linestyle='-')
ax5.spines['right'].set_position(('outward', 180))  # 세 번째 그래프를 오른쪽에 배치
ax5.set_ylabel('최저값과 최댓값이 나온 개수', color='c')
...
for i in range(top_10_percent):
    ...
    ax3.annotate(f'{y3_sorted[-(i+1)]:.2f}', xy=(x1[np.where(y3 == y3_sorted[-(i+1)])][0], y3_sorted[-(i+1)]),
...
for i in range(len(x1)):
    height = y4[i]
    ax4.text(x1[i], height + 0.01, '%d' %height, ha='center', va='bottom', size = 10, color = "brown")
    height1 = y5[i]
    ax5.text(x1[i], height1 + 0.01, '%d' %height1, ha='center', va='bottom', size = 10, color = "c")
